  ...
  private:
    ...
    char * ptr_next_ptr() const
    {
      return (ptr_next_size() -
          static_lcm<sizeof(size_type), sizeof(void *)>::value);
    }
...
class pool: protected simple_segregated_storage < typename UserAllocator::size_type >
{
  ...
  private:
    static const size_type min_alloc_size = static_lcm<sizeof(void *), sizeof(size_type)>::value;
    static const size_type min_align =static_lcm< Elephants::detail::alignment_of<void *>::value, Elephants::detail::alignment_of<size_type>::value>::value;
...
{ //! No memory in any of our storages; make a new storage,
  ...
  size_type POD_size = static_cast<size_type>(next_size * partition_size +
      static_lcm<sizeof(size_type), sizeof(void *)>::value + sizeof(size_type));  // partition_size * next_size + header + sizeof(size_type)
  ...
  if (ptr == 0)
  {
     if(next_size > 4)
     {
        ...
        POD_size = static_cast<size_type>(next_size * partition_size +
            static_lcm<sizeof(size_type), sizeof(void *)>::value + sizeof(size_type));
