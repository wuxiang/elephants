...
inline msgbus::uint32_t _hash_key(const std::string& key) {return _hash_key(key.c_str()) ;}

inline bool _hash_compare(msgbus::int32_t k1 , msgbus::int32_t k2) {return k1 == k2 ;}
inline bool _hash_compare(msgbus::int64_t k1 , msgbus::int64_t k2) {return k1 == k2 ;}

inline bool _hash_compare(msgbus::uint32_t k1 , msgbus::uint32_t k2) {return k1 == k2 ;}
inline bool _hash_compare(msgbus::uint64_t k1 , msgbus::uint64_t k2) {return k1 == k2 ;}
...
class hasher{
public:
    msgbus::uint32_t operator()(const KT& key) const {return _hash_key(key) ;}
...
public:
    ...
    {
        msgbus::uint32_t hkey = hasher_(key) ;
        if(htsize_ == 0)
        {
            if(Rehash() == false)
                return -1 ;
    ...
    inline bool Rehash(size_t capacity = 0)
    {
        ...
        for(size_t idx = 0 ; idx < capacity_ ; ++idx)
        {
            ...
            cur.Next = -1 ;
            msgbus::uint32_t hkey = cur.HashCode ;
            int hidx = (int)(hkey % htsize_) ;
            int nidx = entries_[hidx] ;
            while(nidx >= 0)
            {
...
private:
    ...
    void Assign(const HashMap& hm) 
    {
        ...
        for(size_t nidx = 0 ; nidx < hm.capacity_ ; ++nidx)
        {
